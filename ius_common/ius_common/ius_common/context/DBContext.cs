...
namespace mx.gob.scjn.ius_common.context
{
    public class DBContext
    {
        ...
        private DbConnection GetDBConection()
        {
            ...
            //{
            if ((IUSConstants.IUS_DATABASE_ACCESS != null) && (!IUSConstants.IUS_DATABASE_ACCESS.Equals(IUSConstants.CADENA_VACIA)))
            {
                RealDBConection = new OleDbConnection(IUSConstants.IUS_DATABASE_ACCESS);
        ...
        private DbConnection GetDBConectionEVA()
        {
            //if ((RealDBConection == null)||(RealDBConection.State!=ConnectionState.Open))
            //{
            if ((IUSConstants.IUS_DATABASE_ACCESS_EVA != null) && (!IUSConstants.IUS_DATABASE_ACCESS_EVA.Equals(IUSConstants.CADENA_VACIA)))
            {
                RealDBConection = new OleDbConnection(IUSConstants.IUS_DATABASE_ACCESS_EVA);
                return RealDBConection;
            }
            else
            {
        ...
        public DbCommand CommandRegisterPag(String sql, DbConnection conexion)
        {
            if ((IUSConstants.IUS_DATABASE_ACCESS != null)
                &&(!IUSConstants.IUS_DATABASE_ACCESS.Equals(IUSConstants.CADENA_VACIA)))
            {
                return new OleDbCommand(sql, (OleDbConnection)conexion);
            }
            else
