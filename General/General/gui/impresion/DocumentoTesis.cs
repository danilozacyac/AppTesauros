...
namespace mx.gob.scjn.ius_common.gui.impresion
{
    public class DocumentoTesis
    {
        ...
        public static bool ImprimirLocalizacion { get; set; }
        ...
        public DocumentoTesis(HashSet<int> listadoEnteros, bool Electoral)
        {
#if STAND_ALONE
        ...
        private void generaDocumento(List<TesisTO> todosLosDocumentos)
        {
            ...
            foreach (TesisTO DocumentoActual in todosLosDocumentos)
            {
                ...
                if (ImprimirLocalizacion)
                {
                    ...
                    celdaActual.Padding = new Thickness(0, 0, 48, 0);
                    ...
                    String StrVolumen = valorVolumen + " " + DocumentoActual.Volumen;
                    StrVolumen = StrVolumen.Trim();
                    documentoCopia.Blocks.Add(new Paragraph(new Run(StrVolumen)));
                    celdaActual.FontWeight = FontWeights.Bold;
                    celdaActual.ColumnSpan = 2;
                    //celdaActualcopia.FontWeight = FontWeights.Bold;
                    celdaActual.Padding = new Thickness(0, 0, 48, 0);
                    ...
                    if (DocumentoActual.Pagina.Equals("0"))
                    {
                        celdaActual = new TableCell(new Paragraph(new Run("")));
                    ...
                    columnaInicial.Width = new GridLength(96 * 1.1);
                    ...
                    if ((DocumentoActual.IdGenealogia != null) && (Int32.Parse(DocumentoActual.IdGenealogia) != 0))
                    {
                        ...
                        char[] Enter = { '\n' };
                        String[] genealogias = genealogiaObj.TxtGenealogia.Split(Enter);
                        celdaActual = new TableCell();
                        parrafoGenealogia = new Paragraph();
                        Paragraph parrafoGenealogiaCopia = new Paragraph();
                        parrafoGenealogia.TextAlignment = TextAlignment.Justify;
                        parrafoGenealogia.Inlines.Add(new Run(genealogiaObj.TxtGenealogia));   
                        foreach (String itemStringG in genealogias)
                        {
                            //parrafoGenealogia.Inlines.Add(new Run(itemStringG));
                            parrafoGenealogiaCopia.Inlines.Add(new Run(itemStringG));
                            //parrafoGenealogiaCopia.TextAlignment = TextAlignment.Justify;
                            parrafoGenealogiaCopia.Inlines.Add(new LineBreak());
                        }
                        documentoCopia.Blocks.Add(parrafoGenealogiaCopia);
                        celdaActual.Blocks.Add(parrafoGenealogia);
                ...
                documentoCopia.Blocks.Add(new Paragraph());
...
#if STAND_ALONE
                List<OtrosTextosTO> observaciones = fachada.getOtrosTextosPorIus(DocumentoActual.Ius);
#else
